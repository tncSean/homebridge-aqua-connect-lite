...
export class PoolStateStore extends EventEmitter {
    ...
    ingest(payload: Buffer): void {
        ...
        if (isFrameType(payload, FrameType.LEDS)) {
            this.ingestLeds(payload);
            return;
        }
        // KEEP_ALIVE, PUMP_SPEED_REQUEST, key events: currently unused.
    }

    /**
     * Clear cached heater setpoint so that a subsequent wait-for-change
     * receives a fresh read from the panel's next DISPLAY_UPDATE rather
     * than resolving instantly on a stale value. Used by the thermostat
     * accessory just before it navigates into the heater submenu.
     */
    clearHeaterSetpoint(): void {
        if (this.state.heaterSetpointF !== undefined) {
            this.state = { ...this.state, heaterSetpointF: undefined };
            this.emit('change', 'heaterSetpointF', undefined, this.state);
        }
    }
    ...
    private parseDisplayText(raw: string): void {
        ...
        //   "Pool Heater1            XX°F"    → submenu setpoint readout
        //   "Pool Heater1           Manual Off" → top-level heater view
        ...
        if (/Heater\s*1?\b/i.test(text)) {
            ...
            // Direct submenu setpoint readout — "Pool Heater1  XX°F". This is
            // what the panel displays while we're inside the heater submenu;
            // distinct from "SET TO XX" which was the old firmware's read-only
            // status cycle. Must run BEFORE the word-only branches because the
            // "°F" suffix confirms this is a setpoint display, not an alert.
            const directSetptM = text.match(/Heater\s*1\b[^\d]*?(\d{2,3})\s*(?:°|_|\xdf)?F/i);
            if (directSetptM) {
                const f = parseInt(directSetptM[1], 10);
                if (Number.isFinite(f) && f >= 50 && f <= 110) {
                    this.setField('heaterSetpointF', f);
                    // Don't override 'heating' (LED-derived) — only set 'auto'
                    // if we don't already know the heater is firing.
                    if (this.state.heaterMode !== 'heating') {
                        this.setField('heaterMode', 'auto');
                    }
                }
            } else if (/\bHeating\b/i.test(text)) {
    ...
    /**
     * LEDS frame body is an 8-byte bitmap of indicator LEDs on the panel.
     * Byte 0 bit 0 tracks Heater1 — 1 when the controller considers the
     * heater armed/firing, 0 when off. Confirmed by capture 2026-04-18:
     * byte 0 flipped 0x28 → 0x29 the moment Sean set a setpoint from the
     * menu, and back when he pressed MINUS past "65°F" into "Off".
     *
     * The LED-derived heater mode takes precedence over display-derived
     * 'auto' because the LED is updated in near real-time, whereas display
     * text only refreshes on menu cycles.
     */
    private ingestLeds(payload: Buffer): void {
        const body = payload.subarray(2);
        if (body.length < 1) return;
        const b0 = body[0];
        const heaterLedOn = (b0 & 0x01) !== 0;
        const sp = this.state.heaterSetpointF;
        if (heaterLedOn) {
            this.setField('heaterMode', 'heating');
        } else if (typeof sp === 'number') {
            // Armed (setpoint numeric) but LED off → control loop decided
            // water is already warm enough, so relay is idle. HomeKit sees
            // this as target=HEAT, current=OFF.
            this.setField('heaterMode', 'auto');
        } else if (sp === 'off') {
            this.setField('heaterMode', 'off');
        }
